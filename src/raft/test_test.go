...
func TestFailNoAgree2B(t *testing.T) {
	...
	cfg.logger.Infof("server %v %v %v disconnect", (leader + 1) % servers, (leader + 2) % servers, (leader + 3) % servers)
...
func TestConcurrentStarts2B(t *testing.T) {
	...
	cfg.logger.Infof("-------------------TestConcurrentStarts2B START-----------------")
...
loop:
	...
	cfg.logger.Infof("-------------------TestConcurrentStarts2B ENDS-----------------")
	cfg.end()
}
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	cfg.logger.Infof("server %v disconnect", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	cfg.logger.Infof("server %v disconnect", leader2)

	// old leader connected again
	cfg.connect(leader1)
	cfg.logger.Infof("server %v reconnect", leader1)
...
func TestBackup2B(t *testing.T) {
	...
	cfg.logger.Infof("server %v %v %v disconnect", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	cfg.logger.Infof("server %v %v disconnect", (leader1 + 0) % servers, (leader1 + 1) % servers)

	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	cfg.connect((leader1 + 3) % servers)
	cfg.connect((leader1 + 4) % servers)
	cfg.logger.Infof("server %v %v %v reconnect", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	cfg.logger.Infof("server %v disconnect", other)

	// lots more commands that won't commit
	cfg.logger.Infof("-------------------leader %v BEGIN 50 times Start------------------", leader2)
	for i := 0; i < 50; i++ {
		cfg.rafts[leader2].Start(rand.Int())
	}
	cfg.logger.Infof("-------------------leader %v BEGIN 50 times END------------------", leader2)
	...
	cfg.logger.Infof("disconnect every server")
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)
	cfg.logger.Infof("server %v %v %v reconnect", (leader1 + 0)%servers, (leader1 + 1)%servers, other)

	// lots of successful commands to new group.
	cfg.logger.Infof("-------------------cfg.one() BEGIN 50 times--------------------")
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	cfg.logger.Infof("-------------------cfg.one() END 50 times--------------------")

	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	cfg.logger.Infof("every server is reconnected")
	cfg.one(rand.Int(), servers, true)
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	nup := servers
	for iters := 0; iters < 1000; iters++ {
		cfg.logger.Infof("iters is %v", iters)
		if iters == 200 {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			cfg.logger.Infof("server %v disconnect", leader)
			nup -= 1
		}
		...
		if nup < 3 {
			...
			if cfg.connected[s] == false {
				cfg.connect(s)
				cfg.logger.Infof("server %v reconnect", s)
				nup += 1
			}
		}
	}
	...
	for i := 0; i < servers; i++ {
		if cfg.connected[i] == false {
			cfg.connect(i)
			cfg.logger.Infof("server %v connect", i)
		}
	}
